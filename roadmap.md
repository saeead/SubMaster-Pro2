...
### 8. validation دقیق‌تر خروجی مدل [تا حد زیادی انجام شده]

اعتبارسنجی فعلی بهبود یافته است:

- تشخیص ترجمه‌های تکراری پشت‌سرهم (انجام شده).
- تشخیص عدم تناسب طول ترجمه با duration (انجام شده با هشدار Diagnostic).
...
## کارهای انجام‌شده — کیفیت ترجمه و Prompt

- **بهبودهای Round 1 (Fidelity / Spoken Tone / Orthography):**
  - افزودن "Priority Hierarchy" در دستورالعمل‌های پایه برای جلوگیری از خلاصه‌سازی مدل در زمان محدودیت کاراکتر. اولویت‌ها: اول معنا و لحن کامل، دوم زبان طبیعی، سوم تطبیق با محدودیت‌های خروجی.
  - اصلاح دستورات مربوط به لحن محاوره‌ای تهرانی (Spoken Tehrani Tone) جهت حفظ شکل رسمی کلمات تخصصی، ادبی و رسمی و عدم شکستن ماشینی آن‌ها.
  - تقویت و گسترش قوانین نیم‌فاصله (ZWNJ) در فایل `skeleton_str/index.ts` با یک لیست بزرگ‌تر از افعال پرکاربرد (مانند می‌روم، می‌خواند، ساختم، گرفت و...)، پیشوندها و پسوندها. الگوی RegEx آن اکنون دقیق‌تر عمل کرده و خط‌تیره، فاصله اضافی یا tatweel را قبل از این کلمات حذف و با `U+200C` جایگزین می‌کند.

- **بهبودهای Round 3 (Output Robustness & Skeleton STR Reliability):**
  - تقویت پرامپت‌ها در حالت‌های پیش‌فرض و پاراگراف برای وادار کردن مدل به خروجی فقط JSON آرایه‌ای معتبر بدون هرگونه توضیحات اضافه یا مارک‌داون، به خصوص بعد از دریافت خطا (Retry).
  - اصلاح فرآیند selective retry در Gemini تا پیام خطای دقیق‌تری حاوی جزئیات فرمت‌های ردشده، به پرامپتِ ارسال مجدد تزریق شود.
  - سخت‌گیرانه‌تر شدن قوانین Skeleton STR و تابع `cleanTranslatedSlot` برای نادیده‌گرفتن تایم‌کدهای لو رفته (Leaked timestamps)، مارک‌داون اضافی و جلوگیری قاطع از ابداع تگ‌های جدید یا ادغام خط‌ها.
