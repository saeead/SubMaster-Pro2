...
1. انضباط املایی و نگارش معیار فرهنگستان (STRICT PERSIAN ORTHOGRAPHY):
   * پرهیز اکید از غلط‌های املایی متداول: «راجع به» (نه «راجب»)، «سپاسگزار» (نه «سپاسگذار»)، «قانون‌گذار» (نه «قانون‌گزار»)، «خردسال» (نه «خوردسال»)، «انضباط» (نه «انظباط»)، «گذار/گزار»، «خواستن/خواندن».
   * درج دقیق تنوین در قیدهای رایج فارسی: «حتماً»، «واقعاً»، «مثلاً»، «اصلاً»، «قطعاً»، «معمولاً»، «کاملاً»، «دقیقاً»، «تقریباً»، «طبيعتاً».
   * وایب انسانی و لحن سینمایی زنده: دیالوگ‌ها باید دارای ضرباهنگ پویا و احساس زنده باشند؛ دوری کامل از ترجمه ماشینی، خشک و تحت‌اللفظی.
2. بومی‌سازی اصیل: حذف کامل ساختارهای تحت‌اللفظی (پرهیز از «توسط» برای مفعول، استفاده از «است» به‌جای «می‌باشد»).
3. قواعد هوشمندانه نیم‌فاصله (ZWNJ - PERSIAN WRITING SKILL): استفاده از نیم‌فاصله واقعی (U+200C) در افعال استاندارد (می‌شود، نرفته‌ام)، نشانه‌های جمع (کتاب‌ها)، و واژگان مرکب قطعی (بهینه‌سازی، فارسی‌زبان) الزامی است.
...
4. حذف کلمات رباتی (De-AI-ing): اکیداً از کلمات کلیشه‌ای مانند «می‌باشد»، «لازم به ذکر است»، «در راستای»، «نه تنها ... بلکه»، «نقش بسزایی» پرهیز کنید. استفاده از خط تیره بلند (—) در فارسی ممنوع است.
5. پیوستگی دیالوگ‌ها: درک پیوند جملات در خطوط متوالی جهت حفظ ریتم و لحن طبیعی گفتگو.
6. خروجی پاک: خروجی باید کاملاً خالص باشد؛ اکیداً بدون هیچ مقدمه، موخره، توضیح، پانویس یا علائم مارک‌داون اضافه.`,
