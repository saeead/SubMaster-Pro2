...
export const SUBTITLE_TRANSLATOR_PERSIAN_ORTHOGRAPHY_INSTRUCTION = `دستور خط و نگارش فارسی (PERSIAN WRITING SKILL - استاندارد فرهنگستان):
۱. پاک‌سازی کلمات رباتی (De-AI-ing): استفاده از کلمات کلیشه‌ای هوش مصنوعی نظیر «می‌باشد»، «لازم به ذکر است»، «در راستای»، «نقش بسزایی»، «نه تنها ... بلکه» و خط تیره بلند (—) مطلقاً ممنوع است.
۲. رعایت اجباری نیم‌فاصله واقعی (U+200C):
...
۳. ممنوعیت درج نیم‌فاصله اشتباه (DO NOT OVER-APPLY): افزودن مکانیکی نیم‌فاصله به واژگان محاوره‌ای و پیوسته اکیداً ممنوع است. افعال و کلماتی نظیر «میدن»، «نمیدن»، «نمیشه»، «میزان» باید کاملاً سرهم و بدون هیچ فاصله‌ای نوشته شوند.
۴. حفظ فواصل کلمات مستقل: کلماتی که باید با فاصله کامل (Space) نوشته شوند را با نیم‌فاصله به هم نچسبانید. نمونه‌های صحیح: «که بخش» (نه که‌بخش)، «کسب‌وکار بخش تولید»، «هم بر اساس»، «باهم تصمیم می‌گیرن»، «هم در مورد».
۵. ممنوعیت کامل: هرگز به جای نیم‌فاصله از فاصله معمولی («می روم»)، خط تیره («می-روم») یا چسباندن («بهینهتر») در مواردی که نیم‌فاصله الزامی است استفاده نکنید.
۶. علائم نگارشی فارسی: چسبیده به کلمه قبل با یک فاصله بعد (مانند: «سلام، حالت چطور است؟»). استفاده از «ی» و «ک» استاندارد فارسی.`;
