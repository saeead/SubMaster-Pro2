...
export const normalizeSkeletonPersianHalfSpaces = (value: string): string => value
  ...
  // Attached prefixes without space, or with space/hyphen (e.g. میرود -> می‌رود)
  ...
  // Attached suffixes (ها|های|هایی|تر|ترین) e.g. بهینهتر, بزرگتر, کتابها
  .replace(/(بهینه|بزرگ|کوچک|بلند|کوتاه|بهتر|بیشتر|کمتر|ساده|دشوار|روشن|تاریک)(?:\s|-|\u200C)?(تر|ترین)(?=\s|[.،؛!؟]|$)/gu, `$1${PERSIAN_HALF_SPACE}$2`)
  .replace(/([\u0621-\u06CC]{3,})(?:\s|-|\u200C)?(ها|های|هایی)(?=\s|[.،؛!؟]|$)/gu, `$1${PERSIAN_HALF_SPACE}$2`)
  ...
  .replace(/(دست|کتاب|صفر|نیم|فارسی|دانش|فوق|عکس|بین|هیچ)(?:\s|-|\u200C)+(خط|یاب|نویس|مایه|افزار|کار|آموز|العاده|المللی|کدام|خانه|عرض|فاصله)(?=\s|[.،؛!؟]|$)/gu, `$1${PERSIAN_HALF_SPACE}$2`)
