  ...
  return (
    <div className="glass rounded-3xl p-8 mb-8 animate-in fade-in slide-in-from-bottom-4 relative overflow-visible group">
      ...
      <div className="space-y-6">
        ...
        {/* Global Semantic Context & Memory Blueprint Card */}
        <div className={`rounded-2xl border transition-all p-4 ${
            enableGlobalContextAnalysis
                ? 'dark:border-cyan-500/30 border-cyan-500/40 dark:bg-cyan-950/20 bg-cyan-50/60 shadow-xs'
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            <h4 className="text-sm font-bold text-text">
                                تحلیل موضوعی کل اثر قبل از ترجمه
                            </h4>
                        ...
                        <p className="text-[11px] text-text-muted mt-0.5">
                            مطالعهٔ هوشمند سناریو، درک ژن، لحن و تقسیم سکانس‌ها برای انتقال پیوستهٔ موضوع به مترجم
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-xs font-bold transition-all shadow-xs ${
                            enableGlobalContextAnalysis
                                ? 'bg-cyan-500/15 border-cyan-500/40 text-cyan-600 dark:text-cyan-300 hover:bg-cyan-500/25'
                                : 'bg-slate-200 dark:bg-white/10 border-slate-300 dark:border-white/15 text-text-muted hover:text-text'
                        }`}
                        title={enableGlobalContextAnalysis ? 'کلیک کنید تا تحلیل موضوعی غیرفعال شود' : 'کلیک کنید تا تحلیل موضوعی فعال و نوبتی اجرا شود'}
                    >
                        <div className={`w-2.5 h-2.5 rounded-full transition-colors ${
                            enableGlobalContextAnalysis ? 'bg-cyan-400 shadow-[0_0_8px_rgba(0,240,255,0.8)]' : 'bg-slate-400'
                        }`} />
                        <span>{enableGlobalContextAnalysis ? 'غیرفعال‌سازی' : 'فعال‌سازی'}</span>
                    </button>
                </div>
            </div>

            {/* Body State when Enabled */}
            {enableGlobalContextAnalysis && (
                <div className="space-y-3 pt-1">
                    {/* Analyzing Loader State */}
                    {(activeFile.isAnalyzingContext || isAnalyzingAnyContext) && (
                        <div className="rounded-xl border border-cyan-500/30 bg-cyan-500/10 p-3 text-xs animate-pulse flex items-center justify-between gap-3">
                            <div className="flex items-center gap-2.5 text-cyan-400">
                                <RefreshCw className="w-4 h-4 animate-spin flex-shrink-0" />
                                <span>
                                    {activeFile.isAnalyzingContext
                                        ? `در حال مطالعهٔ اثر «${activeFile.name}»، درک هوشمند موضوع و ساخت فایل حافظه...`
                                        : 'در حال پردازش نوبتی صف تحلیل فایل‌ها و ایجاد حافظهٔ روایی...'}
                                </span>
                            </div>
                    ...
                    {/* Active Semantic Context Blueprint */}
                    {activeFile.semanticContext ? (
                        <div className="rounded-xl border dark:border-cyan-500/20 border-cyan-500/30 dark:bg-black/20 bg-white/70 p-3 text-xs space-y-2">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                                <div className="flex flex-wrap items-center gap-2">
                                    <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-cyan-500/15 text-cyan-600 dark:text-cyan-400 font-bold border border-cyan-500/25">
                                        <Sparkles className="w-3.5 h-3.5 text-cyan-400" />
                                        <span>ژانر: {activeFile.semanticContext.detectedGenre || 'درک موضوعی اثر'}</span>
                                    </span>
                                    ...
                                    <span className="text-[11px] text-text-muted">
                                        🧠 {activeFile.semanticContext.sections.length} سکانس در حافظهٔ هوش مصنوعی
                                    </span>
                                </div>
                                ...
                                <div className="flex items-center gap-2 mr-auto">
                                    <button
                                        type="button"
                                        onClick={() => onForceAnalyzeContext?.(activeFile.id)}
                                        disabled={activeFile.isAnalyzingContext || isAnalyzingAnyContext}
                                        className="flex items-center gap-1 px-2.5 py-1 rounded-lg border dark:border-white/10 border-slate-200 dark:bg-white/5 bg-white text-[11px] text-text-muted hover:text-text hover:border-cyan-500/40 transition-all disabled:opacity-50"
                                        title="تحلیل مجدد سناریوی این فایل با مدل هوش مصنوعی"
                                    >
                                        <RefreshCw className="w-3 h-3" />
                                        <span>تحلیل مجدد این فایل</span>
                                    </button>
                                    ...
                                    {totalFiles > 1 && (
                                        <button
                                            ...
                                            disabled={isAnalyzingAnyContext}
                                            className="flex items-center gap-1 px-2.5 py-1 rounded-lg border border-cyan-500/30 bg-cyan-500/10 text-[11px] text-cyan-600 dark:text-cyan-400 hover:bg-cyan-500/20 transition-all disabled:opacity-50"
                                            title="اجرای تحلیل نوبتی و ساخت فایل حافظه برای تمام فایل‌های پروژه"
                                        >
                                            <Brain className="w-3 h-3" />
                                            <span>تحلیل نوبتی تمام فایل‌ها ({totalFiles})</span>
                                        </button>
                                    )}
                                </div>
                            </div>

                            <p className="text-text-muted leading-relaxed line-clamp-2 text-[11px] border-t dark:border-white/5 border-slate-100 pt-2">
                                <strong className="text-text font-medium">خلاصه کل اثر: </strong>
                                {activeFile.semanticContext.overallSummary}
                            </p>
                        </div>
                    ) : (
                        !activeFile.isAnalyzingContext && (
                            <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-xs flex flex-wrap items-center justify-between gap-3">
                                <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400">
                                    <Sparkles className="w-4 h-4 flex-shrink-0" />
                                    <span>فایل حافظهٔ موضوعی برای این فایل هنوز ساخته نشده است.</span>
                                </div>
                                <div className="flex items-center gap-2 mr-auto">
                                    <button
                                        type="button"
                                        onClick={() => onForceAnalyzeContext?.(activeFile.id)}
                                        className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-amber-500 text-black font-bold hover:bg-amber-400 transition-all shadow-xs text-xs"
                                    >
                                        <Brain className="w-3.5 h-3.5" />
                                        <span>تحلیل و ساخت حافظه این فایل</span>
                                    </button>
                                    {totalFiles > 1 && (
                                        <button
                                            type="button"
                                            onClick={() => onForceAnalyzeContext?.()}
                                            className="flex items-center gap-1.5 px-3 py-1 rounded-lg border border-amber-500/40 bg-amber-500/15 text-amber-700 dark:text-amber-300 font-bold hover:bg-amber-500/25 transition-all text-xs"
                                        >
                                            <RefreshCw className="w-3.5 h-3.5" />
                                            <span>تحلیل نوبتی همه فایل‌ها</span>
                                        </button>
                                    )}
                                </div>
                            </div>
            ...
            {/* Subtle notice when disabled */}
            {!enableGlobalContextAnalysis && (
                <div className="flex flex-wrap items-center justify-between gap-2 pt-1 text-xs text-text-muted">
                    <span>تحلیل موضوعی غیرفعال است. با فعال‌سازی، سناریوی هر فایل به نوبت مطالعه شده و فایل مموری ساخته می‌شود.</span>
                    <button
                        type="button"
                        onClick={onToggleGlobalContextAnalysis}
                        className="text-cyan-500 hover:underline font-bold text-xs mr-auto"
                    >
                        فعال‌سازی و تحلیل نوبتی
                    </button>
                </div>
            )}
        </div>
