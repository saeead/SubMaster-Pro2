...
## 27. Editor Enhancement – Editable Original Text, Block Save/Auto-Save & Bi-directional Find & Replace
در این به‌روزرسانی قابلیت‌های ویرایشی در بخش Subtitle Editor با تمرکز بر تصحیح متن ورودی و راحتی کاربر ارتقا یافت:
- **ویرایش متن اصلی (Original Text):** فیلد متن اصلی از حالت متنیِ صرفاً نمایشی (Static Div) به یک فیلد متنی قابل ویرایش (Textarea) تبدیل شد تا کاربر بتواند خطاهای حاصل از تبدیل صوت به متن (Speech-to-Text / ASR) یا اشتباهات املایی متن مبدأ را مستقیماً اصلاح کند.
- **مکانیزم ذخیره و ذخیره خودکار (Save & Auto-save):**
  - با ویرایش متن در هر بلوک (چه متن مبدأ و چه متن ترجمه شده)، بلافاصله دکمهٔ «ذخیره» با آیکون تیک در بالای بلوک ظاهر می‌شود.
  - ذخیره خودکار هنگام خروج فوکوس از بلوک (Blur/Click-away) یا زدن میانبر `Ctrl+S` فعال است و پس از ذخیره‌سازی، نشان بصری وضعیت «ذخیره شد» برای بازخورد کاربر نمایش داده می‌شود.
  - تاریخچهٔ تغییرات (Undo / Redo) برای هر دو فیلد متن اصلی و ترجمه به‌صورت مجزا ثبت و پشتیبانی می‌شود.
- **ارتقای ابزار جستجو و جایگزینی (Find & Replace):**
  - اضافه شدن امکان انتخاب هدف جستجو: «متن ترجمه»، «متن منبع (صوت به متن)» یا «هر دو متن».
  - امکان جایگزینی کلمات اشتباه صوتی (مانند اشتباهات Whisper / STT) در متن منبع به صورت تکی یا در تمام فایل‌ها فراهم گردید.
- **اصلاح تایپوگرافی و فونت فارسی لیبل‌ها و دکمه ذخیره:**
  - رفع ارث‌بری اشتباه کلاس `font-mono` در دکمهٔ «ذخیره» و نشانگر «ذخیره شد» و اتصال مستقیم آن‌ها به فونت استاندارد برنامه (وزیرمتن / `font-persian`).
  - حذف فاصله‌گذاری بین‌حروفی (`tracking-wider`) و تغییرحالت (`uppercase`) از متون فارسی لیبل‌های «(متن اصلی / منبع)» و «(ترجمه فارسی)» و اعمال جهت‌دهی صحیح `dir="rtl"` برای جلوگیری از شکسته‌شدن حروف فارسی.
- **بهبود طراحی و هماهنگی بصری دکمه ذخیره (UI Polish):**
  - بازطراحی کامل دکمه «ذخیره تغییرات» با استایل شیشه‌ای نئونی هماهنگ با تم نرم‌افزار (`#00f0ff` glow)، لبه‌های خمیده مدرن (`rounded-xl`)، میکرو-انیمیشن Hover و کلیک (`active:scale-95`).
- **جابجایی موقعیت دکمه‌های Undo و Redo:**
  - ترتیب قرارگیری دکمه‌های Undo و Redo بر اساس درخواست کاربر با یکدیگر تعویض گردید (Redo در جایگاه اول و Undo در جایگاه دوم قرار گرفت).
- **افزایش عرض افقی منوی انتخاب گروهی بلوک‌ها:**
  - عرض نوار پاپ‌آپ ابزارهای گروهی در پایین صفحه از `max-w-3xl` به `max-w-5xl` ارتقا یافت تا فضای دکمه‌های ۷گانه ابزارها بازتر شده و فشرده نباشد.
- **رفع باگ پس‌زمینه سفید در حالت تاریک «ترجمه فوری متن و دیالوگ»:**
  - مشکل ناشی از کلاس نامعتبر `dark:bg-surface/50` که در مرورگرها به `bg-white` پیش‌فرض سقوط می‌کرد برطرف شد.
  - اعمال رنگ‌های پس‌زمینه تیره هماهنگ (`dark:bg-[#0c1232]`، `dark:bg-[#0a0f2b]` و `dark:bg-[#070b22]`) برای فریم، پنل‌های ورودی/خروجی و بخش کاری، بدون تغییر یا اختلال در پالت تم روشن.
